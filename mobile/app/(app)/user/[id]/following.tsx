...
import { fetchFollowingProfiles } from '@/lib/api';
...
export default function FollowingListScreen() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    setLoading(true);
    setRows(await fetchFollowingProfiles(id));
    setLoading(false);
  }, [id]);
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Seguindo', headerShown: true }} />
      <Screen edges={['bottom']} belowNativeHeader>
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={[styles.empty, { fontFamily: fonts.body }]}>Esta conta ainda não segue ninguém.</Text>
            }
            renderItem={({ item }) => {
